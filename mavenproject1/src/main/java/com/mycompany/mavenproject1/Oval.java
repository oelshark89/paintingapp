...
public class Oval extends AbstractShape {
    ...
    public Oval(Point position, int vR,int hR) {
        super(position);
        this.height=vR;
        this.width=hR;
    }
...
public boolean isLowerLeft(Point p) {
    ...
    public JSONObject toJSONObject() {
       JSONObject jo=new JSONObject();
        ...
        jo.put("verticalRadius", height);
        jo.put("horizontalRadius", width);
        return jo;
    }
    @Override
   public Shape copy(JComboBox<String> chooseshape,Shape shape ){
        Oval oval=new Oval(shape.getPosition(), ((Oval)shape).height, ((Oval)shape).width);
        chooseshape.addItem("Oval "+ noOfOvals);
      noOfOvals++;
        oval.setColor(shape.getColor());
        oval.setFillColor(shape.getFillColor());
